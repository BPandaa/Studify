            ...
            <div key={post._id} className="bg-white rounded-xl shadow-md overflow-hidden relative">
              <div className="relative h-48 overflow-hidden">
                <img
                  ...
                  alt={post.title}
                  className={`w-full h-full object-cover transition-all ${isLocked ? 'blur-sm' : ''}`}
                />
                {isLocked && (
              ...
              <div className="p-5">
                ...
                <h3 className={`text-xl font-bold mb-2 ${isLocked ? 'blur-[2px]' : ''}`}>
                  {post.title}
                </h3>
                <p className={`text-gray-600 mb-4 ${isLocked ? 'blur-[2px]' : ''}`}>
                  {post.excerpt}
                </p>

                {isLocked ? (
                  <a href="#pricing" className="inline-flex items-center px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white text-sm font-medium rounded-lg transition-colors duration-200">
